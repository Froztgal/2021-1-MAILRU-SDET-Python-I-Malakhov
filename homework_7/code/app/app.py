...
@app.route('/put_user/<name>', methods=['GET'])
def put_user_surname_by_name(name):
    mock_host = os.environ['MOCK_HOST']
    mock_port = os.environ['MOCK_PORT']
    surname = None
    try:
        resp = requests.get(f'http://{mock_host}:{mock_port}/put_surname/{name}')
        if resp.status_code == 200:
            surname = resp.json()
...
@app.route('/del_user/<name>', methods=['GET'])
def del_user_by_name(name):
    mock_host = os.environ['MOCK_HOST']
    mock_port = os.environ['MOCK_PORT']
    surname = None
    try:
        resp = requests.get(f'http://{mock_host}:{mock_port}/delete_surname/{name}')
        if resp.status_code == 200:
            surname = resp.json()
